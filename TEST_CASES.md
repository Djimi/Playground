...
| PLAYGROUND-022 | High | At 320, 768, and 1280 CSS px, hover a pin near an edge and let the popup auto-pan before moving the pointer into it and clicking View details. Focus a pin, Tab to View details, Shift+Tab back, Tab forward again, then Tab past the button; also open details with Enter/Space directly on the pin. | Popup stays open through auto-pan and pointer/keyboard transitions; reverse Tab returns to the pin, forward Tab exits to the next map control, and button/direct activation opens details without a map click closing them. | Enrichment interaction regression |
